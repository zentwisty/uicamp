...
const updatePage = async () => {
  ...
  const fruits = fruitsArray.filter((item) => item.nutritions.protein > 1);
  ...
  fruits.forEach(fruit => {
    ...
    const content = document.createTextNode(fruit.name);
    element.append(content);
    document.body.insertBefore(element, gallery);
  });
}
